...
from secom.app.models.user_information_model import UserInformation
...
class UserInformationRepository:
    ...
    async def find_by_user_id(self, user_id: str) -> UserInformation | None:
        stmt = select(UserInformation).where(UserInformation.user_id == user_id)
        result = await self._session.execute(stmt)
        return result.scalar_one_or_none()

    def _to_profile_response(self, row: UserInformation) -> MyPageProfileResponse:
    ...
    async def get_profile(self, user_id: str) -> MyPageProfileResponse | None:
        logger.info("[UserInformationRepository] get_profile | userId=%s", user_id)
        row = await self.find_by_user_id(user_id)
        if row is None:
            return None
        return self._to_profile_response(row)
    ...
    async def save_profile(self, profile: MyPageProfileSchema) -> None:
        ...
        user_stmt = select(User.user_id).where(User.user_id == profile.userId)
        user_result = await self._session.execute(user_stmt)
        if user_result.scalar_one_or_none() is None:
            raise ValueError("사용자를 찾을 수 없습니다.")

        row = await self.find_by_user_id(profile.userId)
        if row is None:
            row = UserInformation(user_id=profile.userId)
