...
@dataclass(frozen=True)
class PassengerName:
    value: str  # nullable ORM → 빈 문자열이 아닌 None은 도메인에서 허용 안 함
    ...
    def from_raw(cls, value: str | None) -> PassengerName:
        if value is None:
            raise ValueError("PassengerName must not be None")
        return cls(value)
...
class Age:
    """
    None = 미기재(unknown). ORM nullable을 도메인 의미로 흡수.
    """
...
class FamilyRelation:
    """
    sib_sp / parch 둘 다 nullable → 각각 None 허용.
    """
    sib_sp: Optional[int]   # siblings / spouses, None = 미기재
    parch:  Optional[int]   # parents / children,  None = 미기재
    ...
    def family_size(self) -> Optional[int]:
        """sib_sp, parch 중 하나라도 미기재면 산출 불가 → None."""
